# Author:  Stanley Yu
# A logic puzzle game where you try to find the hiding cat.
# This game animates using pygame
...
pygame.display.set_caption("Logic Puzzle - Find the Cat")
...
def displayRules():
    messages = ["Try to find the cat hiding in one of the boxes.",
                ...
                "      example:  move from box 2 to box 3",
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if showBtnEnabled and showBtnXStartCoord <= x <= showBtnXStartCoord + showBtnWidth and \
                ...
                # search for a evasive path in a random way via testing for a solution at a random starting point
                random.shuffle(randPathStart)
                for x in randPathStart:  # loops through the random starting positions
                    if randPath(0, x):  # if a path is found via testing random paths
                        pathExistTemp2 = True
                        evasivePath.reverse()
                        break
